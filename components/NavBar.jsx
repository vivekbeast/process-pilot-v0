  ...
  return (
    <div className=' flex flex-col bg-[#3F4F5F] justify-center items-center w-full '>
      ...
      <div  className="flex items-center gap-0">
        ...
        <div>
          <h1 className="fontName text-white text-lg md:text-3xl font-bold">ProcessPilot</h1>
        </div>
      </div>
      ...
      <div className="hidden lg:flex gap-4 text-md rounded-full px-6 py-2 bg-amber-400  transition-all shadow-md hover:shadow-lg">
        <button onClick={()=> {router.push('/login')}} className="text-white cursor-pointer  font-medium transition-colors">
          Log In
        </button>
        <div className=' h-[24px] w-[1px] bg-white border-0 rounded-full border-white'></div>
        <button onClick={()=> {router.push('/register')}} className=" cursor-pointer text-white  font-medium ">
          Signup
        </button>
      </div>
      ...
      {isOpen && (
        <div className="absolute z-50 top-20  left-0 w-full bg-white/95 backdrop-blur-md shadow-lg border-t border-green-200 flex flex-col items-start px-6 py-4 lg:hidden">
          <div className="flex flex-col gap-3 mt-4 w-full z-50">
            <button onClick={()=> {router.push('/login')}} className="text-gray-700 hover:text-green-600 cursor-pointer text-left font-medium transition-colors">
              Log In
            </button>
            <button onClick={()=> {router.push('/register')}} className="bg-amber-400 cursor-pointer text-white px-6 py-3 rounded-lg hover:from-green-700 hover:to-emerald-700 w-full font-medium transition-all shadow-md">
              Signup
            </button>
          </div>
        </div>
